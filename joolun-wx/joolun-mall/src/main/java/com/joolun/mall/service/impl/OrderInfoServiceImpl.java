    ...
    public void notifyOrder(OrderInfo orderInfo) {
        if (CommonConstants.NO.equals(orderInfo.getIsPay())) {//只有未支付订单能操作
            ...
            if (Objects.nonNull(orderInfo.getCouponPrice())) {
                log.info("使用奖学金抵扣");
                BigDecimal userBankMoney = orderInfo.getCouponPrice().multiply(new BigDecimal("10"));
                log.info("扣除用户余额:{}", userBankMoney);
                WxUser user = wxUserService.getById(orderInfo.getUserId());
                BigDecimal subtract = user.getMoney().subtract(userBankMoney);
