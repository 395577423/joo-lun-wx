<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="planList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="标题图片" align="center" prop="titlePic" />
      <el-table-column label="正文内容" align="center" prop="content" />
      <el-table-column label="价格" align="center" prop="price" />
      <el-table-column label="返现金额" align="center" prop="cashReturn" />
      <el-table-column label="计划开始日期" align="center" prop="startTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="计划结束日期" align="center" prop="endTime" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="标题图片" prop="titlePic">
          <el-input v-model="form.titlePic" placeholder="请输入标题图片" />
        </el-form-item>
...
<script>
import { listPlan, getPlan, delPlan, addPlan, updatePlan, exportPlan } from "@/api/mall/plan";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除学习奖励计划编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有学习奖励计划数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
